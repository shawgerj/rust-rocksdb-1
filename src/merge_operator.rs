...
pub unsafe extern "C" fn full_merge_callback(
    ...
    let ptr = existing_value as *const u8;

    let oldval: Option<&[u8]> = if ptr.is_null() || existing_value_len == 0 {
	None
    } else {
	Some(unsafe { slice::from_raw_parts(ptr, existing_value_len as usize) })
    };
